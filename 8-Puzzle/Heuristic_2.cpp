...
void create_node (node** head, int puzzle[3][3], int cost, int depth, node* parent);
...
void find_solution (node* n) {
	...
	if (y!=2) {
		...
		if (is_duplicated (puzzle_) != true) {
			create_node (&head, puzzle_, n->depth + get_cost(puzzle_), n->depth, n);
		}
	}
...
int main (void) {
	...
	create_node (&head, puzzle, get_cost(puzzle), 0, NULL);
	find_solution(head);
	return 0;
}
...
int get_cost (int puzzle[3][3]) {
	...
	for (i=0;i<3;i++) {
		for (j=0;j<3;j++) {
			if (puzzle[i][j]!=9) {
				cost += abs((puzzle[i][j]-1)/3,i);
				cost += abs((puzzle[i][j]-1)%3,j);
			}
		}
	}
	return cost;
...
void create_node (node** head, int puzzle[3][3], int cost, int depth, node* parent) {
	...
	// 각 state들을 cost가 낮은 순으로 정렬하는 부분
	...
	node* temp = *head;
	while (temp->next!=NULL && cost>=temp->next->cost) {
		temp = temp->next;
	}
